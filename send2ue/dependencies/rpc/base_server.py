...
KEY_FILE = os.path.join(tempfile.gettempdir(), 'blender', 'send2ue', 'key.json')


def get_keys():
    """
    Gets the key value from disk.
...
def generate_key(port):
    """
    Generates a random key value and save it to disk.
    ...
    def is_authorized(self):
        ...
        for key, value in self.headers.items():
            if key == 'Authorization':
                return value and value == self.auth_key
        return False
...
class BaseServer(SimpleXMLRPCServer):
    def __init__(self, *args, **kwargs):
        AuthenticatedRequestHandler.auth_key = generate_key(port=args[0][-1])
